...
function safetyAnswer(question) {
  ...
  const best = scored[0];
  const confident = best.strongMatched >= 1 && (best.coverage >= 0.5 || best.score >= 3)
...
function allTeamsAnswer() {
  const lines = emergencyTeams.map(t => `${t.no}) ${t.name} — ${t.members.length} عضو\n   ${t.duties[0]}`);
  return {
    reply: `فرق الطوارئ ${emergencyTeams.length} فرق:\n${lines.join('\n')}\n\nاسألني عن أي فريق بالاسم (مثلاً: "مين في فريق الإطفاء؟" أو "مهام فريق الإخلاء").`,
    source: TEAMS_SOURCE,
  };
}
...
module.exports = { answer, normalizeArabic, stats };
