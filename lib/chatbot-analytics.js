...
const ENTITY_RE = [
  ['hazards', /بلاغ|بلاغات|هازارد|هزارد|هاوارد|خطوره|خطورة|الخطور|hazard|haz-|مخاطر متسجل/],
...
const WANT = {
  ...
  breakdown: /لكل قسم|كل قسم|حسب القسم|بالاقسام|توزيع|حسب النوع|كل نوع|الانواع|انواع|حسب الحاله|حسب الخطوره|تفصيل|ترتيب الاقسام/,
...
function scopeOf(ctx, nq) {
  ...
  if (DEPT_SCOPED.includes(role)) {
    const d = String((ctx.user && ctx.user.department) || '');
    return { depts: d ? [d] : [], dept: d, forced: true };
  }
  ...
  return { depts: found, dept: found.length === 1 ? found[0] : '', forced: false };
}
...
function scopeLabel(scope, period) {
  const where = !scope.depts.length ? 'المصنع كله'
    : scope.depts.length === 1 ? `قسم ${scope.depts[0]}`
...
function complianceData(ctx, scope) {
  ...
  const rows = employees.map(e => {
    ...
    return { emp: e, hours: hrs, hazards: hz, trainOk: hrs >= targetHours, hazOk: hz >= targetHazards };
  });
  return { rows, targetHours, targetHazards, quartersElapsed, label: `الربع ${['الأول', 'الثاني', 'الثالث', 'الرابع'][quartersElapsed - 1]} ${now.getFullYear()}` };
}
...
function complianceAnswer(nq, ctx, scope) {
  ...
  if ((WANT.breakdown.test(nq) || WANT.top.test(nq) || WANT.bottom.test(nq)) && !scope.depts.length) {
    ...
    let rows = [...byDept.values()].filter(b => b.n >= 3).map(b => ({
      ...b, score: Math.round(((pct(b.t, b.n) + pct(b.h, b.n)) / 2)),
    })).sort((a, b) => b.score - a.score);
    const worst = WANT.bottom.test(nq);
    if (worst) rows = rows.slice().reverse();
    return {
      reply: `🏆 ${worst ? 'أقل' : 'أحسن'} الأقسام التزامًا بالأهداف — ${c.label} (تدريب ${c.targetHours}س / ${arCount(c.targetHazards, 'بلاغ')} لكل موظف):\n${rows.slice(0, 10).map((b, i) => `${i + 1}) ${b.d}: ${b.score}% — تدريب ${pct(b.t, b.n)}% · بلاغات ${pct(b.h, b.n)}% (${arCount(b.n, 'موظف')})`).join('\n')}`,
      source: SOURCE,
      suggestions: ['مين ناقصه تدريب؟', 'نسبة الالتزام الكلية'],
    };
  }
  ...
  return {
    reply: [
      ...
      `• الالتزام الكلي: ${Math.round((pct(trainOk, total) + pct(hazOk, total)) / 2)}%`,
    ].join('\n'),
    source: SOURCE,
...
function inspectionsAnswer(nq, ctx, period) {
  ...
  const cat = /p\s*2|بي 2|برنامج 2/.test(nq) ? 'P2' : /p\s*1|بي 1|برنامج 1/.test(nq) ? 'P1' : null;

  const secIn = cat ? sections.filter(s => s.category === cat) : sections;
  ...
  if (badCount && /غير مطابق|غير المطابق|مشاكل|ملاحظات|مخالف/.test(nq)) {
    ...
    return {
      reply: `🔍 أصناف غير مطابقة في فحص ${label}${cat ? ` (${cat})` : ''}: ${n2(badCount)}\n${bad.map(r => { const it = itemById.get(r.itemId) || {}; return `• ${it.name || r.itemId} — ${it.department || '—'}${r.notes ? ` — ${short(r.notes, 70)}` : ''}`; }).join('\n')}`,
      source: SOURCE,
    };
  }
  ...
  return {
    reply: [
      `🔍 الفحص الشهري${cat ? ` — برنامج ${cat}` : ''} · ${label}`,
...
function answerAnalytics(question, ctx) {
  ...
  if (SUMMARY_RE.test(nq)) return summaryAnswer(ctx, scope, period);
  ...
  switch (entity) {
    case 'hazards': return hazardsAnswer(nq, ctx, scope, period, question);
    case 'permits': return permitsAnswer(nq, ctx, scope, period, question);
    case 'trainings': return trainingsAnswer(nq, ctx, scope, period);
    case 'drills': return drillsAnswer(nq, ctx, scope, period);
    case 'penalties': return penaltiesAnswer(nq, ctx, scope, period);
    case 'employees': return employeesAnswer(nq, ctx, scope);
    case 'compliance': return complianceAnswer(nq, ctx, scope);
    case 'inspections': return inspectionsAnswer(nq, ctx, period);
    default: return null;
  }
}
